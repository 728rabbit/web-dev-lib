...
var iweb = {
        ...
        select_object.each(function(select_index){
            if(!$(this).parent().parent().hasClass('iweb-selector')){
                ...
                if(iweb_object.isMatch($(this).data('virtual'),1) || iweb_object.isMatch($(this).data('virtual'),true) || isMultiple){
                    ...
                    if(!iweb_object.isValue(virtualSelect)) {
                        virtualSelect = iweb_object.language[iweb_object.default_language]['select'];
                    }
    ...
    post: function(post_data,success_callback,complete_callback,percentage_callback){
        ...
        if ((iweb_object.isValue(post_data) && iweb_object.isValue(post_data.url)) && !iweb_object.processing_status){
            ...
            if(iweb_object.csrf_token.length > 0){
                ...
                if(iweb_object.isValue(post_data.value)) {
                    post_data['value']['X-iToken'] = window.btoa(md5(iweb_object.csrf_token+'#dt'+local_time)+'%'+local_time);
                }
                else {
                    post_data['value'] = {};
                    post_data['value']['X-iToken'] = window.btoa(md5(iweb_object.csrf_token+'#dt'+local_time)+'%'+local_time);
            ...
            $.ajax({
                url: post_data.url,
                type: 'post',
                data: ((iweb_object.isValue(post_data.value))?post_data.value:{}),
                dataType: ((iweb_object.isValue(post_data.type))?post_data.type:'json'),
                xhr: function () {
                    var fileXhr = $.ajaxSettings.xhr();
                    if (fileXhr.upload) {
                        fileXhr.upload.addEventListener('progress', function (e) {
                            if (e.lengthComputable) {
                                if (typeof percentage_callback === 'function'){
                                    percentage_callback(Math.ceil(((e.loaded / e.total) * 100)));
                                }
                            }
                        }, false);
                    }
                ...
                success: function(response_data){
                    ...
                    if (iweb_object.isMatch(post_data.loading,true) || iweb_object.isMatch(post_data.loading,1) || iweb_object.isMatch(post_data.loading,2)){
                        if (!iweb_object.isMatch(post_data.loading,2)){
                            iweb_object.processing(false);
                        }
                    }
                ...
                complete: function(response_data) {
                    if (typeof complete_callback === 'function'){
                        complete_callback(response_data);
                    }
                }
            });
        }
            ...
            var delay_timer = setTimeout(function(){
                iweb_object.post(post_data,success_callback,complete_callback,percentage_callback);
                clearTimeout(delay_timer);
            }, 1000);
        }
    },
    form: function(form_id,type,check_func,success_callback,percentage_callback){
        ...
        if (iweb_object.isExist(form_id)){
            ...
            if(!iweb_object.isValue($(form_id).attr('action')) && iweb_object.isValue(iweb_object.api_url)) {
                $(form_id).attr('action',iweb_object.api_url);
            }
            ...
            $(form_id).ajaxForm({
                ...
                beforeSubmit: function(arr, $form, options){
                    ...
                    iweb_object.processing_status = true;
                    if((!iweb_object.isValue($(form_id).data('loading'))) || iweb_object.isMatch($(form_id).data('loading'),true) || iweb_object.isMatch($(form_id).data('loading'),1) || iweb_object.isMatch($(form_id).data('loading'),2)) {
                        iweb_object.processing(true,70);
                    }
                    if (typeof check_func !== 'function'){
                        return true;
                ...
                success: function(response_data){
                    iweb_object.processing_status = false;
                    if((!iweb_object.isValue($(form_id).data('loading'))) || iweb_object.isMatch($(form_id).data('loading'),true) || iweb_object.isMatch($(form_id).data('loading'),1) || iweb_object.isMatch($(form_id).data('loading'),2)) {
                        if(!iweb_object.isMatch($(form_id).data('loading'),2)) {
                            iweb_object.processing(false);
                        }
                    }
                    if (typeof success_callback === 'function'){
